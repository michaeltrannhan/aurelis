...
enum CoreAudioAggregateOwnershipJournalError: Error, Equatable, LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case let .invalidAggregateUID(uid):
            "Invalid EQMacRep aggregate UID: \(uid)"
...
struct CoreAudioAggregateOwnershipRecord: Codable, Equatable, Sendable {
    ...
    var isValid: Bool {
        guard schemaVersion == Self.currentSchemaVersion,
              aggregateUID.hasPrefix(CoreAudioOrphanedAggregateCleanup.aggregateUIDPrefix) else {
            return false
        }
        let suffix = String(aggregateUID.dropFirst(CoreAudioOrphanedAggregateCleanup.aggregateUIDPrefix.count))
    ...
    private func loadRecords() throws -> [CoreAudioAggregateOwnershipRecord] {
        ...
        do {
            ...
            return payload.records.reversed().filter {
                $0.isValid && seen.insert($0.aggregateUID).inserted
            }.reversed()
...
final class CoreAudioAggregateOwnershipJournal: CoreAudioAggregateOwnershipJournaling, @unchecked Sendable {
    ...
    init(journalURL: URL = CoreAudioAggregateOwnershipJournal.defaultJournalURL()) {
        self.journalURL = journalURL
        persistence = CoreAudioAggregateOwnershipPersistenceActor(journalURL: journalURL)
    }
