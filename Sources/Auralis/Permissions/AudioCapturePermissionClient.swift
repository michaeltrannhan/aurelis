...
enum ApplicationRelaunchError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .notRunningFromApplicationBundle:
            "Auralis is not running from an application bundle, so it cannot safely relaunch itself."
        case .replacementDidNotLaunch:
            "macOS did not confirm that the replacement Auralis process launched."
        }
    }
}
