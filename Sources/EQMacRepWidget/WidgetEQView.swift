...
struct EQMacRepEQWidgetView: View {
    let entry: EQMacRepEntry
    ...
    private var header: some View {
        HStack(spacing: 8) {
            ...
            VStack(alignment: .leading, spacing: 1) {
                ...
                Text(controlsEnabled ? entry.snapshot.statusMessage : "Open EQMacRep to use controls")
                    .font(.caption2)
    ...
    private var footer: some View {
        HStack {
            Text("Drag bands in the app for fine control")
                ...
                .foregroundStyle(.tertiary)
            Spacer(minLength: 0)
            Link(destination: URL(string: "eqmacrep://open")!) {
